...
function productoConMayorValor($inventario) {
    $maxValor = 0;
    $productoMax = '';

    foreach ($inventario as $producto => $detalles) {
        $valorTotal = $detalles['cantidad'] * $detalles['precio'];
    ...
    return '<br>' . $productoMax . ' Valor de producto en inventario ' . $maxValor ;
}

echo $productoMayorValor = productoConMayorValor($inventario);
